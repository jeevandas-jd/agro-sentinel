...
class SatelliteService {
  ...
  static Future<Map<String, dynamic>> analyze(double lat, double lng) async {
    ...
    final raw = await _callGroq(lat, lng, beforeB64, afterB64);

    // ══════════════════════════════════════════════════════════════════════
    //  Core formula:  Trees Lost = Destroyed Area (m²) ÷ Canopy Constant
    // ══════════════════════════════════════════════════════════════════════
    ...
    return {
      ...
      'affected_area_ha': (raw['affected_area_ha'] as num?)?.toDouble() ?? 0.0,
